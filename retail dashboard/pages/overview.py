...
st.line_chart(sales_trend, x="Order Date", y="Sales",color=(255, 0, 0))

# ===================== Profit Trend =====================
st.subheader("💹 Profit Trend Over Time")
profit_trend = df.groupby('Order Date')['Profit'].sum().reset_index()
st.line_chart(profit_trend, x="Order Date", y="Profit",color=(60, 179, 113))
...
st.bar_chart(sales_by_category, x="Category", y="Sales",color=(255, 165, 0))

st.subheader("💰 Profit by Region")
profit_by_region = df.groupby("Region")["Profit"].sum().reset_index()
st.bar_chart(profit_by_region, x="Region", y="Profit",color=(238, 130, 238))

st.subheader("👥 Orders by Segment")
orders_by_segment = df.groupby("Segment")["Order ID"].nunique().reset_index()
st.bar_chart(orders_by_segment, x="Segment", y="Order ID",color=(106, 90, 205))